 ...
 perform fw.f_write_log(
  ...
  if v_cnt is not null then
     ...
     perform fw.f_update_load_info(
        ...
        p_value      := coalesce(fw.f_get_max_value(v_tmp_table,v_delta_fld)));
  else 
     v_res = false;
  end if;
...
ALTER FUNCTION fw.f_load_simple(int8, text, text, bool) OWNER TO "admin";
GRANT ALL ON FUNCTION fw.f_load_simple(int8, text, text, bool) TO public;
GRANT ALL ON FUNCTION fw.f_load_simple(int8, text, text, bool) TO "admin";
