CREATE OR REPLACE FUNCTION fw.f_load_id_exists(p_object_id int8, p_start_date timestamp, p_end_date timestamp)
	RETURNS bool
	LANGUAGE plpgsql
	VOLATILE
AS $$
...
DECLARE
    v_location    text := 'fw.f_load_id_exists';
    v_exists      int;
...
begin
    ...
    v_sql := 'select count(0) from fw.load_info where object_id = ' || p_object_id::text || ' and load_status in (' ||
...
ALTER FUNCTION fw.f_load_id_exists(int8, timestamp, timestamp) OWNER TO "admin";
GRANT ALL ON FUNCTION fw.f_load_id_exists(int8, timestamp, timestamp) TO public;
GRANT ALL ON FUNCTION fw.f_load_id_exists(int8, timestamp, timestamp) TO "admin";
