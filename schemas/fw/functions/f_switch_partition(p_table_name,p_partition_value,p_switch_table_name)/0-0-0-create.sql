CREATE OR REPLACE FUNCTION fw.f_switch_partition(p_table_name text, p_partition_name text, p_switch_table_name text)
	RETURNS void
	LANGUAGE plpgsql
	VOLATILE
AS $$
...
ALTER FUNCTION fw.f_switch_partition(text, text, text) OWNER TO "admin";
GRANT ALL ON FUNCTION fw.f_switch_partition(text, text, text) TO public;
GRANT ALL ON FUNCTION fw.f_switch_partition(text, text, text) TO "admin";
