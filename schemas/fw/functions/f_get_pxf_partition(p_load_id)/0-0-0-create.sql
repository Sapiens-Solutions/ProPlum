CREATE OR REPLACE FUNCTION fw.f_get_pxf_partition(p_load_id int8)
	RETURNS text
	LANGUAGE plpgsql
	SECURITY DEFINER
	VOLATILE
...
DECLARE
  v_location     text := 'fw.f_get_pxf_partition';
	   ...
	   then
         if v_range <= 0 then 
           ...
           if v_delta_field_type is null or v_delta_field_type like 'date%' or v_delta_field_type like 'timestamp%' then
            ...
            v_part_string = fw.f_get_enum_partition(p_field_name := v_delta_field, p_format:= v_delta_field_format, p_start_date := v_extraction_from, p_end_date := v_extraction_to);
           end if;
         end if;
       ...
       then 
         if v_range <= 0 then 
           ...
           if v_bdate_field_type is null or v_bdate_field_type like 'date%' or v_bdate_field_type like 'timestamp%' then
            ...
            v_part_string = fw.f_get_enum_partition(p_field_name := v_bdate_field, p_format:= v_bdate_field_format, p_start_date := v_extraction_from, p_end_date := v_extraction_to);
           end if;
         end if;
       else 
         v_part_string = '';
...
ALTER FUNCTION fw.f_get_pxf_partition(int8) OWNER TO "admin";
GRANT ALL ON FUNCTION fw.f_get_pxf_partition(int8) TO public;
GRANT ALL ON FUNCTION fw.f_get_pxf_partition(int8) TO "admin";
