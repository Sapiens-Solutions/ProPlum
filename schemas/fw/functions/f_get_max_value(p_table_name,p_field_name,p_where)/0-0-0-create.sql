CREATE OR REPLACE FUNCTION fw.f_get_max_value(p_table_name text, p_field_name text, p_where text DEFAULT NULL::text)
	RETURNS text
	LANGUAGE plpgsql
	VOLATILE
AS $$
...
DECLARE
  v_location text := 'fw.f_get_max_value';
  v_sql text; 
  v_datatype text;
  v_where text;
  v_res text;
...
ALTER FUNCTION fw.f_get_max_value(text, text, text) OWNER TO "admin";
GRANT ALL ON FUNCTION fw.f_get_max_value(text, text, text) TO public;
GRANT ALL ON FUNCTION fw.f_get_max_value(text, text, text) TO "admin";
