CREATE OR REPLACE FUNCTION fw.f_grant_select(p_trg_table_name text, p_src_table text)
	RETURNS void
	LANGUAGE plpgsql
	SECURITY DEFINER
	VOLATILE
...
declare 
 v_location text := 'fw.f_grant_select';
 v_result text;
 v_owner  text;
 v_src_table text;
...
begin
 ...
 v_trg_table_name = fw.f_unify_name(p_name := p_trg_table_name);
...
ALTER FUNCTION fw.f_grant_select(text, text) OWNER TO "admin";
GRANT ALL ON FUNCTION fw.f_grant_select(text, text) TO public;
GRANT ALL ON FUNCTION fw.f_grant_select(text, text) TO "admin";
