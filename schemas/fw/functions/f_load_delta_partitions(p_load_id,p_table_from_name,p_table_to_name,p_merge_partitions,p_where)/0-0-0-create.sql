CREATE OR REPLACE FUNCTION fw.f_load_delta_partitions(p_load_id int8, p_table_from_name text, p_table_to_name text, p_merge_partitions bool, p_where text DEFAULT NULL::text)
	RETURNS int8
	LANGUAGE plpgsql
	SECURITY DEFINER
	VOLATILE
...
DECLARE
    v_location          text := 'fw.f_load_delta_partitions';
...
ALTER FUNCTION fw.f_load_delta_partitions(int8, text, text, bool, text) OWNER TO "admin";
GRANT ALL ON FUNCTION fw.f_load_delta_partitions(int8, text, text, bool, text) TO public;
GRANT ALL ON FUNCTION fw.f_load_delta_partitions(int8, text, text, bool, text) TO "admin";
