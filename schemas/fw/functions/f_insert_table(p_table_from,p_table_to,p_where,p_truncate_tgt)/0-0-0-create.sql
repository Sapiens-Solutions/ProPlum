CREATE OR REPLACE FUNCTION fw.f_insert_table(p_table_from text, p_table_to text, p_where text DEFAULT NULL::text, p_truncate_tgt bool DEFAULT false)
	RETURNS int8
	LANGUAGE plpgsql
	SECURITY DEFINER
	VOLATILE
...
DECLARE
    v_location text := 'fw.f_insert_table';
    v_table_from text;
    v_table_to text;
    v_where text;
    v_cnt int8;
BEGIN
    v_table_from = fw.f_unify_name(p_name := p_table_from);
...
ALTER FUNCTION fw.f_insert_table(text, text, text, bool) OWNER TO "admin";
GRANT ALL ON FUNCTION fw.f_insert_table(text, text, text, bool) TO public;
GRANT ALL ON FUNCTION fw.f_insert_table(text, text, text, bool) TO "admin";
