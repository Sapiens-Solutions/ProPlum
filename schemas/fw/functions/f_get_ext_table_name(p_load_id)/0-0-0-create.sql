CREATE OR REPLACE FUNCTION fw.f_get_ext_table_name(p_load_id int8)
	RETURNS text
	LANGUAGE plpgsql
	VOLATILE
AS $$
...
DECLARE
  v_location  text := 'fw.f_get_ext_table_name';
...
BEGIN
  ...
  v_ext_table_name = v_schema_name||'.'||coalesce(fw.f_get_constant('c_ext_table_prefix'),'ext_')||v_table_name||'_'||p_load_id;
...
ALTER FUNCTION fw.f_get_ext_table_name(int8) OWNER TO "admin";
GRANT ALL ON FUNCTION fw.f_get_ext_table_name(int8) TO public;
GRANT ALL ON FUNCTION fw.f_get_ext_table_name(int8) TO "admin";
