CREATE OR REPLACE FUNCTION fw.f_merge_tables(p_table_from_name text, p_table_to_name text, p_where text, p_merge_key _text, p_trg_table text DEFAULT NULL::text)
	RETURNS int8
	LANGUAGE plpgsql
	VOLATILE
AS $$
...
DECLARE
    v_location          text := 'fw.f_merge_tables';
...
BEGIN
    ...
    if p_trg_table is null then 
       ...
       v_buffer_table_name =  fw.f_create_tmp_table(p_table_name := v_table_to_name, p_prefix_name := v_buf_prefix);
    --change owner and grant select to buffer
       ...
       perform fw.f_truncate_table(p_table_name := v_buffer_table_name);
    end if;
...
ALTER FUNCTION fw.f_merge_tables(text, text, text, _text, text) OWNER TO "admin";
GRANT ALL ON FUNCTION fw.f_merge_tables(text, text, text, _text, text) TO public;
GRANT ALL ON FUNCTION fw.f_merge_tables(text, text, text, _text, text) TO "admin";
