-- fw.objects definition

-- Drop table

-- DROP TABLE fw.objects;

CREATE TABLE fw.objects (
	...
	CONSTRAINT fk_delta_mode FOREIGN KEY (delta_mode) REFERENCES fw.d_delta_mode(delta_mode),
	CONSTRAINT fk_extraction_type FOREIGN KEY (extraction_type) REFERENCES fw.d_extraction_type(extraction_type),
	CONSTRAINT fk_load_group FOREIGN KEY (load_group) REFERENCES fw.d_load_group(load_group),
...
-- DROP RULE objects_update ON fw.objects;

CREATE RULE objects_update AS
    ON UPDATE TO fw.objects DO  INSERT INTO fw.objects_log (object_id, object_name, object_desc, extraction_type, load_type, merge_key, delta_field, delta_field_format, delta_safety_period, bdate_field, bdate_field_format, bdate_safety_period, load_method, job_name, responsible_mail, priority, periodicity, load_interval, activitystart, activityend, active, load_start_date, delta_start_date, delta_mode, connect_string, load_function_name, where_clause, load_group, src_date_type, src_ts_type, column_name_mapping, transform_mapping, delta_field_type, bdate_field_type, change_type, change_timestamp, change_username)  SELECT old.object_id,
            old.object_name,
            old.object_desc,
            old.extraction_type,
            old.load_type,
            ...
            now() AS now,
            "current_user"() AS "current_user";
-- DROP RULE objects_insert ON fw.objects;

CREATE RULE objects_insert AS
    ON INSERT TO fw.objects DO  INSERT INTO fw.objects_log (object_id, object_name, object_desc, extraction_type, load_type, merge_key, delta_field, delta_field_format, delta_safety_period, bdate_field, bdate_field_format, bdate_safety_period, load_method, job_name, responsible_mail, priority, periodicity, load_interval, activitystart, activityend, active, load_start_date, delta_start_date, delta_mode, connect_string, load_function_name, where_clause, load_group, src_date_type, src_ts_type, column_name_mapping, transform_mapping, delta_field_type, bdate_field_type, change_type, change_timestamp, change_username)  SELECT new.object_id,
            new.object_name,
            new.object_desc,
            new.extraction_type,
            new.load_type,
...
ALTER TABLE fw.objects OWNER TO "admin";
GRANT ALL ON TABLE fw.objects TO "admin";