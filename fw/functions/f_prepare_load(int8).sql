CREATE OR REPLACE FUNCTION ${target_schema}.f_prepare_load(p_load_id int8)
	RETURNS bool
	LANGUAGE plpgsql
	SECURITY DEFINER
	VOLATILE
AS $$
    ...
    * 2023*/
	...
	DECLARE
	  v_location        text := '${target_schema}.f_prepare_load';
	  v_load_type       text;
	  v_object_id       int8;
    ...
    raise notice '1. Start creating external table with params: p_table_name: {%}, p_load_method: {%}, p_schema_name: {%}, p_prefix: {%}',
        ...
        perform ${target_schema}.f_create_ext_table(
...
ALTER FUNCTION ${target_schema}.f_prepare_load(int8) OWNER TO "${owner}";
GRANT ALL ON FUNCTION ${target_schema}.f_prepare_load(int8) TO public;
GRANT ALL ON FUNCTION ${target_schema}.f_prepare_load(int8) TO "${owner}";
