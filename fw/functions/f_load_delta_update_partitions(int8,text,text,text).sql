CREATE OR REPLACE FUNCTION ${target_schema}.f_load_delta_update_partitions(p_load_id int8, p_table_from_name text, p_table_to_name text, p_where text DEFAULT NULL::text)
	RETURNS int8
	LANGUAGE plpgsql
	SECURITY DEFINER
	VOLATILE
...
DECLARE
    v_location          text := '${target_schema}.f_load_delta_update_partitions';
...
ALTER FUNCTION ${target_schema}.f_load_delta_update_partitions(int8, text, text, text) OWNER TO "${owner}";
GRANT ALL ON FUNCTION ${target_schema}.f_load_delta_update_partitions(int8, text, text, text) TO public;
GRANT ALL ON FUNCTION ${target_schema}.f_load_delta_update_partitions(int8, text, text, text) TO "${owner}";
