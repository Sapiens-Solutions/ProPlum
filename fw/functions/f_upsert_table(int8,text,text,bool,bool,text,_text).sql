DROP FUNCTION if exists ${target_schema}.f_upsert_table(int8, text, text, bool, bool, text);

CREATE OR REPLACE FUNCTION ${target_schema}.f_upsert_table(p_load_id int8, p_table_from_name text, p_table_to_name text, p_delete_duplicates bool DEFAULT false, p_analyze bool DEFAULT true, p_where text DEFAULT NULL::text, p_merge_key _text DEFAULT NULL::_text)
	RETURNS int8
	LANGUAGE plpgsql
	SECURITY DEFINER
	VOLATILE
...
--2. Insert all rows from source table into target table
  ...
  v_merge_key_arr = coalesce(p_merge_key,${target_schema}.f_get_merge_key(p_object_id := v_object_id));
...
ALTER FUNCTION ${target_schema}.f_upsert_table(int8, text, text, bool, bool, text,_text) OWNER TO "${owner}";
GRANT ALL ON FUNCTION ${target_schema}.f_upsert_table(int8, text, text, bool, bool, text,_text) TO public;
GRANT ALL ON FUNCTION ${target_schema}.f_upsert_table(int8, text, text, bool, bool, text,_text) TO "${owner}";
