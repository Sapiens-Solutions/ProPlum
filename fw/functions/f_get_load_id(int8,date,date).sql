CREATE OR REPLACE FUNCTION ${target_schema}.f_get_load_id(p_object_id int8, p_start_date date DEFAULT NULL::date, p_end_date date DEFAULT NULL::date)
	RETURNS int8
	LANGUAGE plpgsql
	VOLATILE
AS $$
...
DECLARE
  v_location text := '${target_schema}.f_get_load_id';
  v_table_name text;
  v_error text;
  v_sql text;
...
begin
   ...
   /*select activitystart is null or
    ...
    if not coalesce(v_act_time, false) then
        perform ${target_schema}.f_write_log('SERVICE', 'Now is not the time to start load object ' || p_object_id, v_location);
        return null;
    end if;
    v_extr_start = coalesce(p_start_date,c_minDate);
    v_extr_end   = coalesce(p_end_date,c_maxDate);
    -- look for existing load_id for load
    v_sql := 'select load_id, extraction_from, extraction_to from ${target_schema}.load_info 
...
ALTER FUNCTION ${target_schema}.f_get_load_id(int8, date, date) OWNER TO "${owner}";
GRANT ALL ON FUNCTION ${target_schema}.f_get_load_id(int8, date, date) TO public;
GRANT ALL ON FUNCTION ${target_schema}.f_get_load_id(int8, date, date) TO "${owner}";
