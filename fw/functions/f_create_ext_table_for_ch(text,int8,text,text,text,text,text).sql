CREATE OR REPLACE FUNCTION ${target_schema}.f_create_ext_table_for_ch(p_table_name text, p_load_id int8, p_load_method text DEFAULT NULL::text, p_connect_string text DEFAULT NULL::text, p_schema_name text DEFAULT NULL::text, p_prefix text DEFAULT NULL::text, p_suffix text DEFAULT NULL::text)
	RETURNS text
	LANGUAGE plpgsql
	SECURITY DEFINER
	VOLATILE
...
DECLARE
  v_location     text := '${target_schema}.f_create_ext_table_for_ch';
  v_ext_t_name   text;
  v_table_name   text;
  v_load_method  text;
  v_sql          text;
...
--   ) from
--   (
--   select *, case when v_load_method = 'gpfdist' or v_load_method = '' then alphabetical_position else ordinal_position end as sorting_column 
--   from
...
--  v_columns = replace(v_columns,'""','"');
  v_sql_conn := replace(v_sql_conn, ';', ''); ---changed
  v_sql :=  ' CREATE WRITABLE EXTERNAL TABLE '  || v_ext_t_name ||' (LIKE '|| v_full_table_name || ') ' ||v_sql_conn ||' DISTRIBUTED RANDOMLY;'; ------- changed
  raise notice 'v_ext_t_name: [%], v_sql_conn: [%]',v_ext_t_name,v_sql_conn;
...
ALTER FUNCTION ${target_schema}.f_create_ext_table_for_ch(text, int8, text, text, text, text, text) OWNER TO "${owner}";
GRANT ALL ON FUNCTION ${target_schema}.f_create_ext_table_for_ch(text, int8, text, text, text, text, text) TO public;
GRANT ALL ON FUNCTION ${target_schema}.f_create_ext_table_for_ch(text, int8, text, text, text, text, text) TO "${owner}";
