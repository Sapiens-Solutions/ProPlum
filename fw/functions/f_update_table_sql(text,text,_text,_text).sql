CREATE OR REPLACE FUNCTION ${target_schema}.f_update_table_sql(p_table_to_name text, p_sql text, p_column_list _text, p_merge_key _text)
	RETURNS int8
	LANGUAGE plpgsql
	SECURITY DEFINER
	VOLATILE
...
ALTER FUNCTION ${target_schema}.f_update_table_sql(text, text, _text, _text) OWNER TO "${owner}";
GRANT ALL ON FUNCTION ${target_schema}.f_update_table_sql(text, text, _text, _text) TO public;
GRANT ALL ON FUNCTION ${target_schema}.f_update_table_sql(text, text, _text, _text) TO "${owner}";
