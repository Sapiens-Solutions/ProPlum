CREATE OR REPLACE FUNCTION ${target_schema}.f_get_load_expression(p_load_id int8)
	RETURNS text
	LANGUAGE plpgsql
	VOLATILE
AS $$
...
DECLARE
  v_location  text := '${target_schema}.f_get_load_expression';
...
ALTER FUNCTION ${target_schema}.f_get_load_expression(int8) OWNER TO "${owner}";
GRANT ALL ON FUNCTION ${target_schema}.f_get_load_expression(int8) TO public;
GRANT ALL ON FUNCTION ${target_schema}.f_get_load_expression(int8) TO "${owner}";
