CREATE OR REPLACE FUNCTION ${target_schema}.f_load_partitions(p_load_id int8, p_table_from_name text, p_table_to_name text, p_merge_partitions bool, p_merge_key _text default null::_text, p_where text DEFAULT NULL::text, p_delete_duplicates bool DEFAULT false)
	RETURNS int8
	LANGUAGE plpgsql
	SECURITY DEFINER
	VOLATILE
...
DECLARE
    v_location          text := '${target_schema}.f_load_partitions';
...
--Load rows from source table (p_table_from_name) into target table (p_table_to_name) iteratively for partition interval 
 ...
 --find object settings
  ...
  v_merge_key   = coalesce(p_merge_key, ${target_schema}.f_get_merge_key(v_object_id));
...
ALTER FUNCTION ${target_schema}.f_load_partitions(int8, text, text, bool, _text, text, bool) OWNER TO "${owner}";
GRANT ALL ON FUNCTION ${target_schema}.f_load_partitions(int8, text, text, bool, _text, text, bool) TO public;
GRANT ALL ON FUNCTION ${target_schema}.f_load_partitions(int8, text, text, bool, _text, text, bool) TO "${owner}";