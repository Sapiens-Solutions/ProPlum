CREATE OR REPLACE FUNCTION ${target_schema}.f_execute_function(p_function_name text, p_load_id int8 DEFAULT NULL::bigint)
	RETURNS bool
	LANGUAGE plpgsql
	VOLATILE
AS $$
...
DECLARE
  v_location text := '${target_schema}.f_execute_function';
  v_sql text; 
  v_res text;
  ...
  v_cnt int8;
BEGIN
  --v_function_name = ${target_schema}.f_unify_name(p_name := p_function_name);
...
ALTER FUNCTION ${target_schema}.f_execute_function(text, int8) OWNER TO "${owner}";
GRANT ALL ON FUNCTION ${target_schema}.f_execute_function(text, int8) TO public;
GRANT ALL ON FUNCTION ${target_schema}.f_execute_function(text, int8) TO "${owner}";
