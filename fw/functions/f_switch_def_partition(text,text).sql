CREATE OR REPLACE FUNCTION ${target_schema}.f_switch_def_partition(p_table_from_name text, p_table_to_name text)
	RETURNS void
	LANGUAGE plpgsql
	VOLATILE
AS $$
...
DECLARE
    v_location text := '${target_schema}.f_switch_def_partition';
...
BEGIN
    ...
    v_table_to_name = ${target_schema}.f_unify_name(p_table_to_name);
    -- Log
...
ALTER FUNCTION ${target_schema}.f_switch_def_partition(text, text) OWNER TO "${owner}";
GRANT ALL ON FUNCTION ${target_schema}.f_switch_def_partition(text, text) TO public;
GRANT ALL ON FUNCTION ${target_schema}.f_switch_def_partition(text, text) TO "${owner}";
