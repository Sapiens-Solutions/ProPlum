CREATE OR REPLACE FUNCTION ${target_schema}.f_create_tmp_table(p_table_name text, p_schema_name text DEFAULT NULL::text, p_prefix_name text DEFAULT NULL::text, p_suffix_name text DEFAULT NULL::text, p_drop_table bool DEFAULT true, p_is_temporary bool DEFAULT false)
	RETURNS text
	LANGUAGE plpgsql
	SECURITY DEFINER
	VOLATILE
...
DECLARE
    v_location      text := '${target_schema}.f_create_tmp_table';
    v_table_name    text;
    v_table_type    text;
...
ALTER FUNCTION ${target_schema}.f_create_tmp_table(text, text, text, text, bool, bool) OWNER TO "${owner}";
GRANT ALL ON FUNCTION ${target_schema}.f_create_tmp_table(text, text, text, text, bool, bool) TO public;
GRANT ALL ON FUNCTION ${target_schema}.f_create_tmp_table(text, text, text, text, bool, bool) TO "${owner}";
