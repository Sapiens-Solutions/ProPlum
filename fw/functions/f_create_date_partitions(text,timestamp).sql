CREATE OR REPLACE FUNCTION ${target_schema}.f_create_date_partitions(p_table_name text, p_partition_value timestamp)
	RETURNS void
	LANGUAGE plpgsql
	VOLATILE
AS $$
...
ALTER FUNCTION ${target_schema}.f_create_date_partitions(text, timestamp) OWNER TO "${owner}";
GRANT ALL ON FUNCTION ${target_schema}.f_create_date_partitions(text, timestamp) TO public;
GRANT ALL ON FUNCTION ${target_schema}.f_create_date_partitions(text, timestamp) TO "${owner}";
