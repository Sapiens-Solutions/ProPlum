...
if __name__ == '__main__':
    ...
    parser.add_argument('--fwschema', required=True, help='Framework schema')
    parser.add_argument('--fwoutdir', required=True, help='Framework files output directory')
    ...
    Path(args.fwoutdir + '\\tables').mkdir(parents=True, exist_ok=True)
    Path(args.fwoutdir + '\\functions').mkdir(parents=True, exist_ok=True)
    Path(args.fwoutdir + '\\sequences').mkdir(parents=True, exist_ok=True)
    Path(args.fwoutdir + '\\extensions').mkdir(parents=True, exist_ok=True)
    Path(args.fwoutdir + '\\servers').mkdir(parents=True, exist_ok=True)

    export_schema(cur, args.fwschema, args.fwoutdir, args.params)
    export_tables(cur, args.fwschema, args.fwoutdir, args.params)
    export_table_data(cur, args.fwschema, args.fwoutdir, args.params)
    export_functions(cur, args.fwschema, args.fwoutdir, args.params)
    export_sequences(cur, args.fwschema, args.fwoutdir, args.params)
    export_extensions(args.fwoutdir)
    export_servers(args.fwoutdir)
